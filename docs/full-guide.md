...
| `MONITOR_SIM_SEED` | 随机对照抽样种子（默认 `42`） | 可选 |
| `MONITOR_SIM_EQUITY` | 海龟纸上本金（默认 `1000000`） | 可选 |
| `MONITOR_DELTA` | 每日监控昨日对照（默认 `true`） | 可选 |
| `MONITOR_STATE_DIR` | 对照快照目录（默认 `data/monitor_state`） | 可选 |
