...
- [新功能] HSI 持仓止损参考：从 `HSI_HOLDINGS`（Actions secret）读取成本价，合并扫描并给出 Turtle sell/keep/buy，持仓始终增强并注入 DeepSeek。
...
- [新功能] HSI lite 增强在 DeepSeek 决策仪表盘之外新增独立 Kimi/Moonshot「点评」段落（`KIMI_API_KEY`，不替换主 LLM）。
...
- [修复] 美股 TradingView 图表链接改为裸 ticker（如 `JPM`），不再错误附加 `NASDAQ:` 前缀。
- [改进] 技术筛选合格股报告为命中股票附加 TradingView 图表链接，独立报告补充股票池/窗口/数据源摘要；Actions 配置检查新增邮件与 HSI 筛选状态。
- [新功能] 技术筛选股票池新增 `HK_ALL` 全港股快照（`resources/universes/hk_all_stocks.json`），可通过 `python scripts/generate_hk_universe.py` 刷新；`HSI` 仍为恒指成分股。
- [新功能] 每日 Actions 新增 `hk-all-only` 运行模式，一键仅执行 `HK_ALL` 全港股技术筛选。
- [新功能] 技术筛选股票池新增 `DOW`、`NASDAQ_TOP`、`US_TOP` 内置静态美股列表，可通过 `REPORT_QUALIFIED_SCAN_STOCK_LIST` 在 HSI 之外切换扫描范围。
- [新功能] 每日 Actions 新增 `us-only` 运行模式与 `us_scan_pool` 选项，支持一键仅执行 `DOW` / `NASDAQ_TOP` / `US_TOP` 美股技术筛选（自动跳过个股 AI 与大盘复盘）。
...
- [新功能] 新增 `--scan-hsi` CLI 子命令，对恒生指数 95 只成分股进行 S1/S2 突破信号扫描，支持 `--scan-period`、`--scan-conditions`、`--scan-output`、`--scan-notify` 参数，结果可通过通知渠道推送。
- [新功能] 新增 `strategies/s1_s2_breakout.yaml` 策略，AI Agent 可基于 S1/S2 突破信号进行趋势分析。
- [新功能] 支持 `STOCK_LIST=HSI` 特殊值，自动展开为恒生指数成分股列表。
- [新功能] HSI 扫描接入多源数据 (`DataFetcherManager`)，支持 AkShare/Tushare 等数据源自动切换，增强港股数据获取可靠性。
- [新功能] HSI 扫描接入交易日历检查，港股休市日自动跳过扫描。
- [新功能] 新增 `bot/commands/hsi.py` 机器人命令 (`/hsi`)，支持在 DingTalk/Feishu/Discord 中触发恒指扫描。
- [新功能] 新增 `api/v1/endpoints/hsi.py` API 接口 (`GET /api/v1/hsi/scan`, `GET /api/v1/hsi/stocks`, `GET /api/v1/hsi/conditions`)，支持通过 API 触发扫描。
- [新功能] 新增 HSI 扫描调度集成，通过 `HSI_SCAN_ENABLED=true` 可在定时任务模式中每 30 分钟自动扫描。
- [新功能] 新增 `.github/workflows/hsi_scan.yml` GitHub Actions 工作流，港股交易时段每小时自动扫描。
- [新功能] 新增 `HSI_SCAN_SCHEDULE_TIME`、`HSI_SCAN_CONDITIONS`、`HSI_SCAN_PERIOD` 等配置项，支持自定义扫描行为。
