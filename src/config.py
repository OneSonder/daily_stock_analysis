...
class Config:
    ...
    # === HSI 扫描配置 ===
    hsi_scan_enabled: bool = False                  # 是否启用定时 HSI 扫描
    hsi_scan_schedule_time: str = "09:30"           # HSI 扫描每日执行时间 (HH:MM)
    hsi_scan_conditions: str = "s1_breakout,s2_breakout"  # 默认扫描条件
    hsi_scan_period: str = "1y"                     # 历史数据周期
    hsi_scan_max_workers: int = 8                   # 并发线程数
    hsi_scan_notify: bool = True                    # 扫描结果是否推送通知
    hsi_scan_check_trading_day: bool = True         # 是否跳过休市日
    ...
    def _load_from_env(cls) -> 'Config':
        ...
        # 特殊值 HSI 展开为恒生指数成分股
        if len(stock_list) == 1 and stock_list[0] == 'HSI':
            try:
                from src.services.hsi_scanner import HSI_STOCKS
                stock_list = [item['code'] for item in HSI_STOCKS]
                logger.info("STOCK_LIST=HSI 已展开为 %d 只恒生指数成分股", len(stock_list))
            except ImportError:
                logger.warning("无法导入 hsi_scanner 模块，STOCK_LIST=HSI 展开失败")
        ...
        return cls(
            ...
            # HSI 扫描配置
            hsi_scan_enabled=os.getenv('HSI_SCAN_ENABLED', 'false').lower() not in ('0', 'false', 'no', 'off'),
            hsi_scan_schedule_time=os.getenv('HSI_SCAN_SCHEDULE_TIME', '09:30'),
            hsi_scan_conditions=os.getenv('HSI_SCAN_CONDITIONS', 's1_breakout,s2_breakout'),
            hsi_scan_period=os.getenv('HSI_SCAN_PERIOD', '1y'),
            hsi_scan_max_workers=int(os.getenv('HSI_SCAN_MAX_WORKERS', '8')),
            hsi_scan_notify=os.getenv('HSI_SCAN_NOTIFY', 'true').lower() not in ('0', 'false', 'no', 'off'),
            hsi_scan_check_trading_day=os.getenv('HSI_SCAN_CHECK_TRADING_DAY', 'true').lower() not in ('0', 'false', 'no', 'off'),
            # Discord 机器人扩展配置
