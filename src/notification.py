...
def build_dashboard_report_filename(stock_count: int, *, when: Optional[datetime] = None) -> str:
    """Dashboard markdown filename: ``report_YYYYMMDD_HHMMSS_stocksN.md``."""
    dt = when or datetime.now()
    return f"report_{dt.strftime('%Y%m%d_%H%M%S')}_stocks{stock_count}.md"


def is_dashboard_report_filename(filename: str) -> bool:
    """True for aggregate dashboard files (timestamp + stock count), not e.g. market_review."""
...
def update_latest_dashboard_pointer(saved_file: Path, *, reports_dir: Optional[Path] = None) -> None:
    """
    Point ``reports/latest_dashboard.md`` at the newest dashboard file.

    Uses a same-directory relative symlink when supported; otherwise writes
    ``reports/.latest_dashboard`` with one absolute path line.
    """
    saved = Path(saved_file).resolve()
    if not is_dashboard_report_filename(saved.name):
        return
    rd = Path(reports_dir).resolve() if reports_dir is not None else saved.parent
    ...
    try:
        ...
        try:
            fallback.write_text(str(saved) + "\n", encoding="utf-8")
        except OSError as exc:
            logger.warning("无法写入 latest 指针 %s: %s", fallback, exc)
...
class NotificationService(
    ...
    def generate_wechat_summary(self, results: List[AnalysisResult]) -> str:
        ...
        lines.extend([
            ...
            f"*{labels['details_report_hint']} {labels['details_report_filename_hint']}*"
        ])
    ...
    def save_report_to_file(
        ...
        Args:
            content: 日报内容
            filename: 文件名（可选；默认 report_YYYYMMDD_HHMMSS.md）
        ...
        try:
            update_latest_dashboard_pointer(filepath, reports_dir=reports_dir)
        except Exception as exc:
            logger.warning("更新 latest_dashboard 指针失败（已忽略）: %s", exc)
...
def send_daily_report(results: List[AnalysisResult]) -> bool:
    ...
    # 保存到本地
    service.save_report_to_file(report, build_dashboard_report_filename(len(results)))
...
if __name__ == "__main__":
    ...
    # 保存到文件
    print("\n=== 保存日报 ===")
    filepath = service.save_report_to_file(report, build_dashboard_report_filename(len(test_results)))
